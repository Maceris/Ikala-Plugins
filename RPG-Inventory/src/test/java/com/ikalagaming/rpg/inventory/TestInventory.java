...
class TestInventory {
    ...
    void testAddItem() {
        ...
        for (int i = 0; i < inventory.getSize(); ++i) {
            Assertions.assertTrue(inventory.addItem(ItemGenerator.getAccessory()));
        }
        ...
        Assertions.assertTrue(inventory.addItem(stackable, maxConsumableStack * 3));
    ...
    void testCanFitItemStack() {
        ...
        Assertions.assertFalse(
                inventory.canFitItem(stackable, InvUtil.maxStackSize(stackable) * 4));
        Assertions.assertTrue(inventory.canFitItem(stackable, InvUtil.maxStackSize(stackable)));
        // Fill stack
        inventory.addItem(stackable, InvUtil.maxStackSize(stackable));
        ...
        inventory.addItem(stackable);
        // barely can't fit
        Assertions.assertFalse(inventory.canFitItem(stackable, InvUtil.maxStackSize(stackable)));
        Assertions.assertTrue(inventory.canFitItem(stackable, InvUtil.maxStackSize(stackable) - 1));
    }
    ...
    void testEquipment() {
        ...
        for (var entry : equipment.entrySet()) {
            ...
            if (EquipmentSlot.RING_RIGHT.equals(slot)) {
            ...
            log.info("Adding item to slot {}", slot);
            Assertions.assertTrue(inventory.isEmpty(slot), "Slot should start empty for " + slot);
            Assertions.assertFalse(
                    inventory.hasItem(slot), "Slot should start without item for " + slot);
            Assertions.assertTrue(
                    inventory.canEquip(entry.getKey()), "We should be able to equip " + slot);
            Assertions.assertTrue(
                    inventory.equip(entry.getKey()), "We should successfully equip " + slot);

            Assertions.assertFalse(
                    inventory.isEmpty(slot), "Slot should not be empty after equiping " + slot);
            Assertions.assertTrue(
                    inventory.hasItem(slot), "Slot should not have item after equiping " + slot);
        }

        for (EquipmentSlot slot : EquipmentSlot.values()) {
            Assertions.assertTrue(
                    inventory.hasItem(slot), "Inventory should have an item in the slot " + slot);
        }

        for (var entry : equipment.entrySet()) {
            Assertions.assertFalse(
                    inventory.canEquip(entry.getKey()),
                    "Full equipment inventory should not be able to equip " + entry.getValue());
        }
        ...
        for (var entry : equipment.entrySet()) {
            ...
            Assertions.assertTrue(
                    inventory.isEmpty(slot), "cleared slot should be empty for " + slot);
            Assertions.assertFalse(
                    inventory.hasItem(slot), "cleared slot should not have an item for " + slot);
            Assertions.assertTrue(
                    inventory.canEquip(entry.getKey()),
    ...
    void testGetItemCount() {
        ...
        Assertions.assertEquals(InvUtil.maxStackSize(stackable), inventory.getItemCount(0));
    ...
    void testSetItemStackable() {
        ...
        Assertions.assertEquals(first, inventory.getItem(0).get());
        Assertions.assertEquals(InvUtil.maxStackSize(first), inventory.getItemCount(0));
    ...
    /** Test {@link Inventory#swapSlots(Inventory, EquipmentSlot, Inventory, EquipmentSlot)}. */
    @Test
    void testSwapEquipmentSlots() {
        ...
        Assertions.assertEquals(
                armor1.getID(), inventory1.getItem(EquipmentSlot.HEAD).get().getID());
        Assertions.assertEquals(
                armor2.getID(), inventory2.getItem(EquipmentSlot.HEAD).get().getID());

        Inventory.swapSlots(inventory1, inventory2, EquipmentSlot.HEAD);
        Assertions.assertEquals(
                armor2.getID(), inventory1.getItem(EquipmentSlot.HEAD).get().getID());
        Assertions.assertEquals(
                armor1.getID(), inventory2.getItem(EquipmentSlot.HEAD).get().getID());
    ...
    /** Test {@link Inventory#swapSlots(Inventory, EquipmentSlot, Inventory, int)}. */
    @Test
    void testSwapEquipmentSlotWithInventory() {
        ...
        Assertions.assertEquals(
                inventory.getItem(EquipmentSlot.HEAD).get().getID(), armor1.getID());
        ...
        Assertions.assertFalse(inventory.isEmpty(EquipmentSlot.HEAD));
        Assertions.assertEquals(
                inventory.getItem(EquipmentSlot.HEAD).get().getID(), armor2.getID());
        Assertions.assertEquals(inventory.getItem(1).get().getID(), armor1.getID());

        // Swap empty slot 0 and armor 2
        Inventory.swapSlots(inventory, EquipmentSlot.HEAD, inventory, 0);
        Assertions.assertFalse(inventory.isEmpty(0));
        Assertions.assertTrue(inventory.isEmpty(EquipmentSlot.HEAD));
        Assertions.assertEquals(inventory.getItem(0).get().getID(), armor2.getID());
        ...
        Assertions.assertTrue(inventory.isEmpty(EquipmentSlot.HEAD));
        Assertions.assertEquals(inventory.getItem(2).get().getID(), weapon.getID());
        ...
        Assertions.assertTrue(inventory.isEmpty(EquipmentSlot.HEAD));
        Assertions.assertEquals(inventory.getItem(3).get().getID(), consumable.getID());
    }

    /**
     * Test negative cases for {@link Inventory#swapSlots(Inventory, EquipmentSlot, Inventory,
     * int)}.
     */
    @Test
    ...
    /** Test {@link Inventory#swapSlots(Inventory, int, Inventory, EquipmentSlot)}. */
    @Test
    ...
    /**
     * Test negative cases for {@link Inventory#swapSlots(Inventory, int, Inventory,
     * EquipmentSlot)}.
     */
    @Test

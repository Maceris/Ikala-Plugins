...
public class ModelLoader {
     ...
     * {@link ModelLoader#MAX_WEIGHTS} entries in each list, and if there are less than that number
     * of bones affecting the vertex, the remaining positions are filled with zeroes.
     *
    ...
    private static Deque<ModelLoadRequest> loadQueue = new ConcurrentLinkedDeque<>();
    ...
    private static void buildFrameMatrices(
            ...
            @NonNull Model.AnimatedFrame animatedFrame,
            int frame,
            @NonNull Node node,
        ...
        AINodeAnim aiNodeAnim = ModelLoader.findAIAnimNode(aiAnimation, nodeName);
        ...
        for (Node childNode : node.getChildren()) {
            ModelLoader.buildFrameMatrices(
                    aiAnimation,
                    boneList,
                    animatedFrame,
                    frame,
    ...
    private static Node buildNodesTree(@NonNull AINode aiNode, Node parentNode) {
        ...
        Node node = new Node(nodeName, parentNode, ModelLoader.toMatrix(aiNode.mTransformation()));
     ...
     */
    private static AINodeAnim findAIAnimNode(
    ...
    public static Model loadModel(@NonNull ModelLoadRequest request) {
        ...
        for (int i = 0; i < numMaterials; ++i) {
            ...
            Material material = ModelLoader.processMaterial(aiMaterial, modelDir);
        ...
        Matrix4f globalInverseTransformation =
                ModelLoader.toMatrix(aiScene.mRootNode().mTransformation()).invert();
        if (numAnimations > 0) {
            nodeID.set(0);
            Node rootNode = ModelLoader.buildNodesTree(aiScene.mRootNode(), null);

            animations =
                    ModelLoader.processAnimations(
                            aiScene, boneList, rootNode, globalInverseTransformation);
        }
    ...
    private static List<Model.Animation> processAnimations(
        ...
        for (int i = 0; i < numAnimations; ++i) {
            ...
            Model.Animation animation =
                    new Model.Animation(animationName, aiAnimation.mDuration(), frames);
    ...
    /**
     * Process bitangent (points in the direction of the positive Y texture axis) values for the
     * mesh, and turn it into a list of float values that we can use for OpenGL.
     *
     * @param aiMesh The mesh to process.
     * @param normals A list of x, y, z values (in that order) of all the normals.
     * @return A list of x, y, z values (in that order) of all the bitangents.
     */
    private static float[] processBitangents(@NonNull AIMesh aiMesh, float[] normals) {
     ...
     */
    private static AnimMeshData processBones(@NonNull AIMesh aiMesh, @NonNull List<Bone> boneList) {
        ...
        for (int i = 0; i < numBones; ++i) {
            ...
            Bone bone = new Bone(id, boneName, ModelLoader.toMatrix(aiBone.mOffsetMatrix()));
            boneList.add(bone);
            ...
            for (int j = 0; j < numWeights; j++) {
                ...
                VertexWeight vw =
                        new VertexWeight(bone.boneID(), aiWeight.mVertexId(), aiWeight.mWeight());
                List<VertexWeight> vertexWeightList =
                        weightSet.computeIfAbsent(vw.vertexID(), ignored -> new ArrayList<>());
        ...
        return new AnimMeshData(
                Utils.listFloatToArray(weights),
    ...
    private static Material processMaterial(
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            int[] pMax = {1};
            result =
                    Assimp.aiGetMaterialFloatArray(
                            aiMaterial,
     ...
     *     OpenGL.
     */
    private static MeshData processMesh(@NonNull AIMesh aiMesh, @NonNull List<Bone> boneList) {
    ...
    /**
     * Process the normals data from a mesh and turn it into a list of float values that we can use
     * for OpenGL.
     *
     * @param aiMesh The mesh to process.
    ...
    /**
     * Process tangent (points in the direction of the positive X texture axis) values for the mesh,
     * and turn it into a list of float values that we can use for OpenGL.
     *
     * @param aiMesh The mesh to process.
     * @param normals A list of x, y, z values (in that order) of all the normals.
     * @return A list of x, y, z values (in that order) of all the tangents.
     */
    private static float[] processTangents(@NonNull AIMesh aiMesh, @NonNull float[] normals) {
    ...
    /**
     * Process the texture coordinates for the first (0 index) texture for a mesh, converts it to a
     * buffer of x and then 1-y values for each coordinate.
     *
     * @param aiMesh The mesh to process.
     * @return An array of coordinate values for all texture coordinates, in x, y order.
     */
    ...
    /**
     * Process the vertex data from a mesh and convert it to a float buffer that we can use for
     * OpenGL.
     *
     * @param aiMesh The mesh to process.

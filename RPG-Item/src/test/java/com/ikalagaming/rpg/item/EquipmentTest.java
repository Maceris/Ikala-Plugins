...
class EquipmentTest {
     ...
     */
    private AttributeModifier findModifier(
            List<AttributeModifier> modifiers, Attribute attribute, ModifierType type) {
        for (AttributeModifier modifier : modifiers) {
            if (attribute.equals(modifier.getAttribute()) && type.equals(modifier.getType())) {
                return modifier;
            }
        }
        return null;
     ...
     * @return The attribute, or null.
     */
    private DamageModifier findModifier(
            List<DamageModifier> modifiers, DamageType damage, ModifierType type) {
        for (DamageModifier modifier : modifiers) {
            if (damage.equals(modifier.getDamageType()) && type.equals(modifier.getType())) {
                return modifier;
            }
        }
        return null;
    ...
    void testCombineStats() {
        ...
        equipment.getItemStats().getDamageBuffs().add(electricDamagePercentEquip);
        ...
        equipment.getItemStats().getResistanceBuffs().add(unholyResistPercentEquip);
        ...
        expectedCharismaFlat.setAmount(
                charismaFlat1.getAmount()
                        + charismaFlat2.getAmount()
                        + charismaFlatEquip.getAmount());

        // Stat in all 3
        Assertions.assertEquals(
                expectedCharismaFlat,
                this.findModifier(
                        combined.getAttributeBuffs(), Attribute.CHARISMA, ModifierType.FLAT));
        ...
        Assertions.assertEquals(
                expectedCharismaPercent,
                this.findModifier(
                        combined.getAttributeBuffs(), Attribute.CHARISMA, ModifierType.PERCENTAGE));
        ...
        Assertions.assertEquals(
                expectedIntFlat,
                this.findModifier(
                        combined.getAttributeBuffs(), Attribute.INTELLIGENCE, ModifierType.FLAT));
        ...
        Assertions.assertEquals(
                expectedDexFlat,
                this.findModifier(
                        combined.getAttributeBuffs(), Attribute.DEXTERITY, ModifierType.FLAT));
        ...
        expectedFireDmgFlat.setAmount(fireDamageFlat1.getAmount() + fireDamageFlat2.getAmount());

        // Damage buff only on both affixes
        Assertions.assertEquals(
                expectedFireDmgFlat,
                this.findModifier(combined.getDamageBuffs(), DamageType.FIRE, ModifierType.FLAT));
        ...
        expectedFireDmgPercent.setAmount(
                fireDamagePercent1.getAmount()
                        + fireDamagePercent2.getAmount()
                        + fireDamagePercentEquip.getAmount());

        // Damage buff on all 3
        Assertions.assertEquals(
                expectedFireDmgPercent,
                this.findModifier(
                        combined.getDamageBuffs(), DamageType.FIRE, ModifierType.PERCENTAGE));
        ...
        Assertions.assertEquals(
                expectedPoisonDmgPercent,
                this.findModifier(
                        combined.getDamageBuffs(), DamageType.POISON, ModifierType.PERCENTAGE));
        ...
        expectedElectricDmgPercent.setAmount(electricDamagePercentEquip.getAmount());

        // Damage buff only on one affix
        Assertions.assertEquals(
                expectedElectricDmgPercent,
                this.findModifier(
                        combined.getDamageBuffs(), DamageType.ELECTRIC, ModifierType.PERCENTAGE));
        ...
        expectedHolyFlat.setAmount(
                holyResistFlat1.getAmount()
                        + holyResistFlat2.getAmount()
                        + holyResistFlatEquip.getAmount());
        ...
        Assertions.assertEquals(
                expectedHolyPercent,
                this.findModifier(
                        combined.getResistanceBuffs(), DamageType.HOLY, ModifierType.PERCENTAGE));
        ...
        expectedPsychichPercent.setAmount(
                psychicResistPercent1.getAmount() + psychicResistPercent2.getAmount());

        // Damage resist only on both affixes
        Assertions.assertEquals(
                expectedPsychichPercent,
                this.findModifier(
                        combined.getResistanceBuffs(),
                        DamageType.PSYCHIC,
        ...
        Assertions.assertEquals(
                expectedUnholyPercent,
                this.findModifier(
                        combined.getResistanceBuffs(), DamageType.UNHOLY, ModifierType.PERCENTAGE));
    }
}

...
class ItemRollerTest {
     ...
     * @return Whether or not the modifier could have been generated from the template.
     */
    private static boolean attributeModifierValid(
            final AttributeModifierTemplate template, final AttributeModifier actual) {
    ...
    /**
     * Check that the list of modifier templates could have generated the actual list of modifiers.
     *
     * @param template The template list.
     * @param actual The actual list.
     */
    private static void matchAttributeTemplateLists(
            List<AttributeModifierTemplate> template, List<AttributeModifier> actual) {
                                ...
                                expectedAttribute ->
                                        actual.stream()
                                                .anyMatch(
                                                        actualAttribute ->
                                                                ItemRollerTest
                                                                        .attributeModifierValid(
                                                                                expectedAttribute,
                                                                                actualAttribute))),
                "Expected everything in "
                        + ItemRollerTest.gson.toJson(template)
                        + " to be in "
                        + ItemRollerTest.gson.toJson(actual));
                        ...
                        .allMatch(
                                expectedDamage ->
                                        actual.stream()
                                                .anyMatch(
                                                        actualDamage ->
                                                                ItemRollerTest.damageModifierValid(
                                                                        expectedDamage,
                                                                        actualDamage))));
        // For each attribute, there exists a template that could generate it
        Assertions.assertTrue(
    ...
    /**
     * Match equipment fields to make sure they could ahve been generated from the template.
     *
     * @param template The template for generating the equipment.
     * @param actual The actual equipment.
     */
    private static void matchEquipmentFields(EquipmentTemplate template, Equipment actual) {
        ItemRollerTest.matchItemFields(template, actual);
        Assertions.assertEquals(template.getLevelRequirement(), actual.getLevelRequirement());

        ItemRollerTest.matchAttributeTemplateLists(
                template.getAttributeRequirements(), actual.getAttributeRequirements());

        ItemRollerTest.matchItemStats(template.getItemStatsTemplate(), actual.getItemStats());
    }
    ...
    /**
     * Match the item stats template to make sure the actual stats could have been generated from
     * them.
     *
     * @param template The stats template.
     * @param actual The actual stats.
     */
    private static void matchItemStats(ItemStatsTemplate template, ItemStats actual) {
        ItemRollerTest.matchAttributeTemplateLists(
                template.getAttributeBuffs(), actual.getAttributeBuffs());
        ItemRollerTest.matchDamageTemplateLists(template.getDamageBuffs(), actual.getDamageBuffs());
        ItemRollerTest.matchDamageTemplateLists(
                template.getResistanceBuffs(), actual.getResistanceBuffs());
    }
    ...
    void testRollAccessory() {
        ...
        Assertions.assertEquals(template.getAccessoryType(), accessory.getAccessoryType());
    }
    ...
    void testRollWeapon() {
        ...
        Assertions.assertEquals(template.getWeaponType(), weapon.getWeaponType());

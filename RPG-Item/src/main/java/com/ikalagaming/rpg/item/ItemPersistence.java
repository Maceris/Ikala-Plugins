...
public class ItemPersistence {
    ...
    private static final Table<?> AFFIX_TABLE = DSL.table(DSL.name(Affix.TABLE_NAME));

    private static final ItemCriteriaConverter itemCriteriaConverter = new ItemCriteriaConverter();

    private static final ItemStatsConverter itemStatsConverter = new ItemStatsConverter();
    ...
    public void fetchAffix() {
        ...
        List<org.jooq.Record> affixes = context.select().from(ItemPersistence.AFFIX_TABLE).fetch();
        ...
        affixes.forEach(
                entry -> {
                    ...
                    affix.setAffixType(AffixType.valueOf((String) entry.get("AFFIX_TYPE")));
                    affix.setItemCriteria(
                            ItemPersistence.itemCriteriaConverter.convertToEntityAttribute(
                                    (String) entry.get("ITEM_CRITERIA")));
                    affix.setQuality(Quality.valueOf((String) entry.get("QUALITY")));
                    affix.setItemStats(
                            ItemPersistence.itemStatsConverter.convertToEntityAttribute(
                                    (String) entry.get("ITEM_STATS")));

                    catalog.getAffixes().add(affix);
                });
    }

    /** Load the database context from the database plugin, for use in the rest of the methods. */
    public void loadContext() {
        Optional<Plugin> plugin = PluginManager.getInstance().getPlugin("Ikala-Database");
        if (plugin.isEmpty()) {
            // TODO localize

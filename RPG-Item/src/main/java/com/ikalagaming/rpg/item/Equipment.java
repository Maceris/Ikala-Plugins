...
public class Equipment extends Item {
    ...
    /**
     * A unique identifier to refer to a specific piece of equipment, since they are always unique
     * and never stack.
     */
    ...
    /**
     * Go through a new set of stats and add them to the combined stats attribute buffs list.
     *
    ...
    private void addAttributeBuffs(ItemStats newStats) {
        for (AttributeModifier newBuff : newStats.getAttributeBuffs()) {
            ...
            for (AttributeModifier existingBuff : combinedStats.getAttributeBuffs()) {
                if (existingBuff.getAttribute().equals(newBuff.getAttribute())
                        && existingBuff.getType().equals(newBuff.getType())) {
                    existingBuff.setAmount(existingBuff.getAmount() + newBuff.getAmount());
                    matched = true;
                    break;
                }
            }
            if (!matched) {
                combinedStats.getAttributeBuffs().add(newBuff.copy());
            }
        }
    }

    /**
     * Go through a new set of stats and add them to the combined stats damage buffs list.
     *
     * @param newStats The stats we are adding to this item.
     */
    private void addDamageBuffs(ItemStats newStats) {
        for (DamageModifier newBuff : newStats.getDamageBuffs()) {
            boolean matched = false;
            for (DamageModifier existingBuff : combinedStats.getDamageBuffs()) {
            ...
            if (!matched) {
                combinedStats.getDamageBuffs().add(newBuff.copy());
            }
        }
    }

    /**
     * Go through a new set of stats and add them to the combined stats resistance buffs list.
     *
     * @param newStats The stats we are adding to this item.
     */
    private void addResistanceBuffs(ItemStats newStats) {
        for (DamageModifier newBuff : newStats.getResistanceBuffs()) {
            boolean matched = false;
            for (DamageModifier existingBuff : combinedStats.getResistanceBuffs()) {
                if (!(existingBuff.getDamageType().equals(newBuff.getDamageType())
                        && existingBuff.getType().equals(newBuff.getType()))) {
                    continue;
                }
            ...
            if (!matched) {
                combinedStats.getResistanceBuffs().add(newBuff.copy());
            }
        }
    }
    ...
    private void addStats(@NonNull ItemStats newStats) {
        ...
        addAttributeBuffs(newStats);
        addDamageBuffs(newStats);
        addResistanceBuffs(newStats);
    }

    /**
     * Return the combined stats that include all base item stats and affixes. Calculates it if it
     * has not been calculated yet, but caches the result.
     *
    ...
    public ItemStats getCombinedStats() {
        ...
        List<AttributeModifier> attributeBuffs = combinedStats.getAttributeBuffs();
        List<DamageModifier> damageBuffs = combinedStats.getDamageBuffs();
        List<DamageModifier> resistanceBuffs = combinedStats.getResistanceBuffs();

        for (AttributeModifier modifier : getItemStats().getAttributeBuffs()) {
            attributeBuffs.add(modifier.copy());
        }
        for (DamageModifier modifier : getItemStats().getDamageBuffs()) {
            damageBuffs.add(modifier.copy());
        }
        for (DamageModifier modifier : getItemStats().getResistanceBuffs()) {
            resistanceBuffs.add(modifier.copy());
        }

        if (prefix != null) {
            addStats(prefix.getItemStats());
        }
        if (suffix != null) {
            addStats(suffix.getItemStats());
        }
        if (gem1 != null) {
            addStats(gem1.getItemStats());
        }
        if (gem2 != null) {
            addStats(gem2.getItemStats());
        }
        if (augment != null) {
            addStats(augment.getItemStats());
        }

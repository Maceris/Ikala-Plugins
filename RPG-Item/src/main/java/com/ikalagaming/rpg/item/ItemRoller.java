...
public class ItemRoller {
    ...
    /**
     * Copies both item fields, and equipment fields, into the first object from the second. Rolls
     * fields as required.
     *
     * @param modified The item to copy item and equipment fields into, which is modified.
     * @param template The item to copy from, not modified.
     */
    private static void copyEquipmentFields(Equipment modified, final EquipmentTemplate template) {
        ItemRoller.copyItemFields(modified, template);
        template.getAttributeRequirements()
                .forEach(
                        modifier ->
                                modified.getAttributeRequirements()
                                        .add(ItemRoller.rollAttributeModifier(modifier)));
        modified.setLevelRequirement(template.getLevelRequirement());
        modified.setItemStats(ItemRoller.rollItemStats(template.getItemStatsTemplate()));
    }
    ...
    /**
     * Generate an amount +/- some variance. Variance should be between 1 and amount, we will take
     * the absolute value of the variance if negative.
     *
     * @param amount The amount.
     * @param variance The distance from the amount we can generate values within.
     ...
     */
    private static AttributeModifier rollAttributeModifier(AttributeModifierTemplate template) {
        AttributeModifier modifier = new AttributeModifier();
        modifier.setAmount(ItemRoller.rollAmount(template.getAmount(), template.getVariance()));
     ...
     */
    private static DamageModifier rollDamageModifier(DamageModifierTemplate template) {
